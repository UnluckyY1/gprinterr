...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  else if([@"printLabelmini" isEqualToString:call.method]) {
     @try {
       NSDictionary *args = [call arguments];
       [Manager write:[self mapToTscCommandmini:args]];
       result(nil);
     } @catch(FlutterError *e) {
       result(e);
     }
...
-(NSData *)mapToTscCommand:(NSDictionary *) args {
    ...
    for(NSDictionary *m in list){
        ...
        if([@"text" isEqualToString:type]){
            ...
            [command addBitmapwithX:[x intValue] withY:[y intValue] withMode:0 withWidth:500 withImage:image];
        }
...
-(NSData *)mapToTscCommandmini:(NSDictionary *) args {
    NSDictionary *config = [args objectForKey:@"config"];
    NSMutableArray *list = [args objectForKey:@"data"];
